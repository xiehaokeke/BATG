...
traindata = torchvision.datasets.ImageFolder(c.TRAIN_PATH, transform=tfms)
trainloader = DataLoader(dataset=traindata, shuffle=True, batch_size=16, num_workers=2, pin_memory=False, drop_last=False)
testdata = torchvision.datasets.ImageFolder(c.VAL_PATH, transform=tfms)
testloader = DataLoader(dataset=testdata, shuffle=True, batch_size=4, num_workers=1, pin_memory=False, drop_last=False)
...
secret = Image.open(f'./secret/airplane.png').convert('RGB')
print("image airplane is trigger!")
#secret = Image.open(f'./secret/61_2.png').convert('RGB')
#print("image traffic signal is trigger!")
secret = tfms(secret)
secret = secret.reshape(-1, 3, size, size).to(device)
val_batch = secret
train_batch = secret

if c.tain_next:
    print("%s have been loaded!" % c.next)
...
try:
    ...
    for i_epoch in range(c.epochs):
        ...
        for i_batch, (data, _) in enumerate(trainloader):
            ...
            secret = train_batch.clone().repeat(ts, 1, 1, 1)#
            ...
            jpeg_steg_img = JpegCompression(device=device, yuv_keep_weights=(16, 9, 9))(steg_img)#
        ...
        if i_epoch % c.val_freq == 0:
            with torch.no_grad():
                ...
                for (x, _) in testloader:
                    ...
                    secret = val_batch.clone().repeat(vs, 1, 1, 1)#
                    cover_input = dwt(cover)
                    secret_input = dwt(secret)
